...
class _CrearOracionScreenState extends State<CrearOracionScreen> {
  ...
  void initState() {
    super.initState();
    cargarFrasesGuardadas().then((_) => aprenderDeFrases());
    inicializarPictogramas();
    cargarContadorUso();
  }

  void aprenderDeFrases() {
    for (var frase in frasesGuardadas) {
      final palabras = frase.split(' ');
      for (int i = 0; i < palabras.length - 1; i++) {
        final actual = palabras[i];
        final siguiente = palabras[i + 1];
        coocurrencias.putIfAbsent(actual, () => {});
        coocurrencias[actual]![siguiente] =
            (coocurrencias[actual]![siguiente] ?? 0) + 1;
      }
    }
  }
  ...
  Future<void> inicializarPictogramas() async {
    ...
    List<Pictograma> base = [
      ...
      Pictograma(
        ...
        imagen:
            'assets/Pictogramas/Deportes_juegos/jugador de hockey hielo.png',
        categoria: 'Deportes_juegos',
      ),
      Pictograma(
        nombre: 'Escaladora',
      ...
      Pictograma(
        ...
        imagen:
            'assets/Pictogramas/Deportes_juegos/jugar a la videoconsola.png',
        categoria: 'Deportes_juegos',
      ),
      Pictograma(
        nombre: 'Juegos',
  ...
  Widget sugerenciaWidget() {
    if (seleccionados.isEmpty) return SizedBox();
    final ultimo = seleccionados.last.nombre;
    final sugerencias = coocurrencias[ultimo];
    if (sugerencias == null || sugerencias.isEmpty) return SizedBox();

    final siguienteMasComun = sugerencias.entries
        .reduce((a, b) => a.value >= b.value ? a : b)
        .key;

    final pictogramaSugerido = pictogramas.firstWhere(
      (p) => p.nombre == siguienteMasComun,
      orElse: () => Pictograma(nombre: siguienteMasComun, imagen: ''),
    );
  ...
  Widget build(BuildContext context) {
    ...
    for (var p in pictogramas) {
      final c = p.categoria ?? 'Sin categoría';
